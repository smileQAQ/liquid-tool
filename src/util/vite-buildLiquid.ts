...
async function copyTemplateFiles(templateDir: string, targetDir: string, replacements: Record<string, string>) {
...
export async function viteGenerateLiquid(options: { dir: string; output: string; dom: string }) {
  ...
  const config = await fs.promises.readFile(path.join(dir, 'config.json'), 'utf-8');
...
export async function getProjectData(options: { dir: string }) {
  const { dir } = options;
  const config = await fs.promises.readFile(path.join(process.cwd(), `/src/pages/${dir}`, 'config.json'), 'utf-8');
  return config;
}
